...
public class Game extends JFrame {
     ...
     * getMissle();
     */
    public void fireMissle(Tower shooter, Unit target) {
        double speedMissle = 0; // = shooter.getMissle().getSpeed();
        ...
        for(int i = target.getPathCounter() + 1; i < path.length; i++){
            ...
            if (speedMissle >= (Math.sqrt(Math.pow(Math.abs(tower.getX() - unit.getX()), 2) + Math.pow(Math.abs(tower.getY() - unit.getY()), 2))
                    / ((1.0 / speedTarget) * count ))) {
                //vector missle @ m.getField(unit)
                break;
            }
            count++;
        }
